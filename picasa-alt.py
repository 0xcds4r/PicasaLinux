...
class PhotoViewer(Gtk.Window):
	def __init__(self):
		...
		open_btn.connect("clicked", self.load_folder)
		...
		self.flowbox.connect("query-tooltip", self.on_flowbox_query_tooltip)
	...
	def load_folder(self, widget=None):
	...
	def create_thumbnail(self, path):
		try:
			...
			w = pixbuf.get_width()
			h = pixbuf.get_height()
			size = min(w, h, self.thumbnail_size)
			scale = size / max(w, h)
...
if __name__ == "__main__":
	...
	if len(sys.argv) > 1:
		win.current_folder = sys.argv[1]
		win.image_files = [
			os.path.join(win.current_folder, f) 
			for f in os.listdir(win.current_folder) 
			if f.lower().endswith(('png', 'jpg', 'jpeg', 'gif', 'bmp'))
		]
